...
public class RCCP_Installation {
    ...
    public static void CheckProjectLayers() {
        ...
        if (!layer_RCC || !layer_RCC_DetachablePart || !layer_RCC_WheelCollider || !layer_RCC_Prop || !layer_RCC_Obstacle) {
            ...
            // Add required layers automatically instead of showing a modal dialog.
    ...
    public static void CheckAllLayers() {
        ...
        // Resolve script GUIDs for every RCCP component type that needs a layer
        // assigned. Searching prefab YAML for these GUIDs is ~8x faster than
        // calling LoadAssetAtPath per prefab — text I/O bypasses Unity's full
        // asset-loading pipeline (mesh / material / dependency inflation).
        // Combining all four type scans into a single parallel pass over the
        // project is what makes this practical on big projects.
        string vehicleScriptGuid = FindScriptGuid("RCCP_CarController");
        string partScriptGuid = FindScriptGuid("RCCP_DetachablePart");
        string propScriptGuid = FindScriptGuid("RCCP_Prop");
        string trailerScriptGuid = FindScriptGuid("RCCP_TrailerController");

        // Fall back to the legacy per-type loaders if any GUID lookup failed
        // (renamed scripts, deleted .cs, broken .meta) so the install still
        // completes — just at the old slow speed.
        if (vehicleScriptGuid == null || partScriptGuid == null ||
            propScriptGuid == null || trailerScriptGuid == null) {
        ...
        ConcurrentBag<string> vehiclePaths = new ConcurrentBag<string>();
        ConcurrentBag<string> partPaths = new ConcurrentBag<string>();
        ConcurrentBag<string> propPaths = new ConcurrentBag<string>();
        ConcurrentBag<string> trailerPaths = new ConcurrentBag<string>();
        ...
        try {
            ...
            for (int batchStart = 0; batchStart < paths.Length; batchStart += batchSize) {

                int batchEnd = System.Math.Min(batchStart + batchSize, paths.Length);

                if (EditorUtility.DisplayCancelableProgressBar(
                        "Searching for RCCP Prefabs..",
                        $"Scanned {batchStart}/{paths.Length} prefabs. Found {vehiclePaths.Count} vehicles, {partPaths.Count} parts, {propPaths.Count} props, {trailerPaths.Count} trailers.",
                        batchStart / (float)paths.Length)) {
                ...
                int rangeStart = batchStart;
                int rangeEnd = batchEnd;
                ...
                Parallel.For(rangeStart, rangeEnd, i => {
                    ...
                    if (content.Contains(vehicleScriptGuid)) vehiclePaths.Add(path);
                    if (content.Contains(partScriptGuid)) partPaths.Add(path);
                    if (content.Contains(propScriptGuid)) propPaths.Add(path);
                    if (content.Contains(trailerScriptGuid)) trailerPaths.Add(path);
        ...
        // Apply layers per type. Each candidate list is now scoped to prefabs
        // whose YAML actually references the matching script, so LoadAssetAtPath
        // runs on tens of prefabs instead of thousands. Same write order as the
        // legacy per-type methods so overlapping prefabs (e.g. a DetachablePart
        // child inside a vehicle) get the expected last-write-wins layer.
        if (!cancelled) {

            ApplyVehicleLayers(vehiclePaths);
            ApplyDetachablePartLayers(partPaths);
            ApplyTrailerLayers(trailerPaths);
            ApplyPropLayers(propPaths);
        ...
        // CollisionMatrix is idempotent and independent of prefab modifications;
        // run it even if the scan was cancelled so the physics matrix is still
        // configured for any RCCP layers that CheckProjectLayers added earlier.
        CollisionMatrix();

    }

    /// <summary>
    /// Returns the GUID of a MonoScript whose file name matches typeName + ".cs",
    /// or null if not found. Used by the fast layer-scan path to identify
    /// component scripts via prefab YAML text-search.
    /// </summary>
    private static string FindScriptGuid(string typeName) {

        string[] guids = AssetDatabase.FindAssets(typeName + " t:MonoScript");
        string suffix = "/" + typeName + ".cs";

        for (int i = 0; i < guids.Length; i++) {

            string p = AssetDatabase.GUIDToAssetPath(guids[i]);

            if (!string.IsNullOrEmpty(p) && p.EndsWith(suffix))
                return guids[i];
    ...
    private static void ApplyVehicleLayers(IEnumerable<string> paths) {

        int vehicleLayer = LayerMask.NameToLayer(RCCP_Settings.Instance.RCCPLayer);
        int wheelLayer = LayerMask.NameToLayer(RCCP_Settings.Instance.RCCPWheelColliderLayer);
        int partLayer = LayerMask.NameToLayer(RCCP_Settings.Instance.RCCPDetachablePartLayer);

        foreach (string path in paths) {

            RCCP_CarController target = AssetDatabase.LoadAssetAtPath<RCCP_CarController>(path);

            if (!target)
                continue;

            target.gameObject.layer = vehicleLayer;

            foreach (Transform item in target.GetComponentsInChildren<Transform>(true))
                item.gameObject.layer = vehicleLayer;

            foreach (RCCP_WheelCollider item in target.GetComponentsInChildren<RCCP_WheelCollider>(true))
                item.gameObject.layer = wheelLayer;

            foreach (RCCP_DetachablePart item in target.GetComponentsInChildren<RCCP_DetachablePart>(true))
                item.gameObject.layer = partLayer;

        }

    }

    private static void ApplyDetachablePartLayers(IEnumerable<string> paths) {

        int partLayer = LayerMask.NameToLayer(RCCP_Settings.Instance.RCCPDetachablePartLayer);

        foreach (string path in paths) {

            RCCP_DetachablePart target = AssetDatabase.LoadAssetAtPath<RCCP_DetachablePart>(path);

            if (!target)
                continue;

            target.gameObject.layer = partLayer;
    ...
    private static void ApplyPropLayers(IEnumerable<string> paths) {

        int propLayer = LayerMask.NameToLayer(RCCP_Settings.Instance.RCCPPropLayer);

        foreach (string path in paths) {

            RCCP_Prop target = AssetDatabase.LoadAssetAtPath<RCCP_Prop>(path);

            if (!target)
                continue;

            target.gameObject.layer = propLayer;

            foreach (Transform item in target.GetComponentsInChildren<Transform>(true))
                item.gameObject.layer = propLayer;

        }

    }

    private static void ApplyTrailerLayers(IEnumerable<string> paths) {

        int trailerLayer = LayerMask.NameToLayer(RCCP_Settings.Instance.RCCPLayer);

        foreach (string path in paths) {

            RCCP_TrailerController target = AssetDatabase.LoadAssetAtPath<RCCP_TrailerController>(path);

            if (!target)
                continue;

            target.gameObject.layer = trailerLayer;

            foreach (Transform item in target.GetComponentsInChildren<Transform>(true))
                item.gameObject.layer = trailerLayer;
