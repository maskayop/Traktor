...
public class RCCP_AddonDefineManager : AssetPostprocessor {
    ...
    private struct AddonInfo {
        [Tooltip("Scripting define symbol added when this addon is detected.")]
        public string define;
        [Tooltip("Folder path used to detect the addon (relative to RCCP base or absolute).")]
        public string relativeOrStaticPath;
        [Tooltip("True if the path is an absolute Assets/ path rather than relative to RCCP.")]
    ...
    private static readonly AddonInfo[] addons = new[]{
        ...
        new AddonInfo{
            define               = "BCG_RCCP_AI",
            relativeOrStaticPath = "Assets/RCCP AI Assistant",
            isStaticPath         = true
        }
    };
