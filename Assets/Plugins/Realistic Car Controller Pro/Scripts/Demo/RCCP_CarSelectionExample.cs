...
public class RCCP_CarSelectionExample : RCCP_GenericComponent {
    ...
    private void CreateVehicles() {
        ...
        // Demo vehicles registry ships with the Demo Content addon — bail out with a clear
        // message instead of throwing when it's not imported.
        RCCP_DemoVehicles demoVehicles = RCCP_DemoVehicles.Instance;

        if (demoVehicles == null || demoVehicles.vehicles == null) {

            Debug.LogError("RCCP_DemoVehicles registry not found. Import the Demo Content package (Welcome Window > Demos) before using the car selection example.");
            return;

        }

        for (int i = 0; i < demoVehicles.vehicles.Length; i++) {
            ...
            RCCP_CarController spawnedVehicle = RCCP.SpawnRCC(demoVehicles.vehicles[i], spawnPosition.position, spawnPosition.rotation, false, false, false);
