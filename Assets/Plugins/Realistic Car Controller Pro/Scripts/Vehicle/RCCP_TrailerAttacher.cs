...
public class RCCP_TrailerAttacher : RCCP_Component {
    ...
    /// </summary>
    [Tooltip("Reference to the trailer currently attached via this attacher (null if none).")]
    public RCCP_TrailerController attachedTrailer;

    private void OnTriggerEnter(Collider col) {

        // Check if the other collider has a trailer attacher.
        if (!col.gameObject.TryGetComponent<RCCP_TrailerAttacher>(out var otherAttacher))
            return;
