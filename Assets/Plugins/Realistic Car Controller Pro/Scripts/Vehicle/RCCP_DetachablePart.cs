...
public class RCCP_DetachablePart : RCCP_Component {
    ...
    /// </summary>
    [Tooltip("Optional center of mass override; if assigned, the part's rigidbody COM is moved here at Awake")]
    ...
    /// </summary>
    [Tooltip("Identifies this panel's role on the vehicle, used for damage-weight multipliers")]
    public DetachablePartType partType = DetachablePartType.Hood;

    /// <summary>
    /// If true, the Configurable Joint motions will be locked at Start.
    /// </summary>
    [Tooltip("Lock all Configurable Joint motions at Start so the part stays rigidly attached until damaged")]
    public bool lockAtStart = true;

    /// <summary>
    /// Overall strength of the part. Decreases with collisions until it detaches or becomes loose.
    /// </summary>
    [Tooltip("Current structural integrity; decreases on each collision until the part loosens or detaches")]
    ...
    /// </summary>
    [Tooltip("Whether this part can fully separate from the vehicle once strength drops below the detach point")]
    public bool isDetachable = true;

    /// <summary>
    /// Strength threshold at which the part becomes "loose" but not fully detached.
    /// </summary>
    [Tooltip("Strength threshold at which the joint unlocks and the part begins to wobble")]
    [Min(0)] public int loosePoint = 50;

    /// <summary>
    /// Strength threshold at which the part fully detaches from the vehicle.
    /// </summary>
    [Tooltip("Strength threshold at which the part fully detaches and becomes a free rigidbody")]
    [Min(0)] public int detachPoint = 0;

    /// <summary>
    /// After full detachment, the part is disabled after this many seconds.
    /// </summary>
    [Tooltip("Seconds after full detachment before the part GameObject is deactivated")]
    ...
    /// </summary>
    [Tooltip("Local-space torque applied each frame when loose, simulating the part flapping at speed")]
    ...
    private void Reset() {
        ...
        if (!TryGetComponent<ConfigurableJoint>(out var cJoint))
        ...
        if (cJoint.TryGetComponent<Rigidbody>(out var cJointRigid)) {
