...
public class RCCP_Limiter : RCCP_Component {
    ...
    /// </summary>
    [Header("Speed Limits")]
    [Tooltip("Maximum allowed speed (km/h) per gear index; -1 means no limit for that gear.")]
    ...
    /// </summary>
    [Tooltip("Automatically populates speed limits from each gear's theoretical top speed in the gearbox.")]
    public bool autoSet = false;

    /// <summary>
    /// If true, the limiter is actively preventing further acceleration (engine cut).
    /// </summary>
    [Tooltip("Read-only flag indicating the limiter is currently cutting fuel to enforce the speed cap.")]
    ...
    /// </summary>
    [Header("Downhill Force")]
    [Tooltip("Applies a braking counter-force when the vehicle exceeds the limit on a downhill slope.")]
    ...
    /// </summary>
    [Tooltip("Strength of the retarding force (N/kg) used to hold speed on downhill grades.")]
