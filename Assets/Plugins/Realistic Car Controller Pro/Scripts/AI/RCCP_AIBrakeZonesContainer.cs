...
public class RCCP_AIBrakeZonesContainer : RCCP_GenericComponent {
    ...
    ///  Brake Zones list.
    /// </summary>
    [Tooltip("All brake zones contained in this group (auto-populated from children).")]
    ...
    private void OnDrawGizmos() {
        ...
        for (int i = 0; i < brakeZones.Count; i++) {
            ...
            if (brakeZones[i] != null && brakeZones[i].gameObject.activeSelf) {
                ...
                brakeZones[i].TryGetComponent<BoxCollider>(out var zoneCollider);
                Vector3 colliderBounds = zoneCollider != null ? zoneCollider.size : Vector3.one;
