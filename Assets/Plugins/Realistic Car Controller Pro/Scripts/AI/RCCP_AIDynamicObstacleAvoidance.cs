...
public class RCCP_AIDynamicObstacleAvoidance : RCCP_Component {
    ...
    // Cached Physics.OverlapSphere buffer. The non-alloc overload writes into a reused array
    // rather than allocating one per call. Buffer grows when fills hit capacity.
    private Collider[] obstacleHitsBuffer = new Collider[32];
    ...
    public override void Awake() {
        ...
        //  V2.51 (T1-13): a LayerMask of Nothing (0) makes every OverlapSphere/Raycast return empty, so
        //  obstacle avoidance silently does nothing. Warn so the user knows to set the Dynamic Obstacle Layers.
        if (dynamicObstacleLayers.value == 0)
            Debug.LogWarning("RCCP: Dynamic Obstacle Layers is set to Nothing on '" + name + "'. Obstacle avoidance will detect nothing - assign the layers your obstacles/vehicles use.", this);
    }
    ...
    private void RefreshObstaclePool() {
        // Grow the buffer when a fill saturates capacity (OverlapSphereNonAlloc truncates
        // silently when results don't fit). Stable size after the first few frames.
        int hitCount;
        while (true) {
            hitCount = Physics.OverlapSphereNonAlloc(CarController.transform.position, detectionRadius, obstacleHitsBuffer, dynamicObstacleLayers);
            if (hitCount < obstacleHitsBuffer.Length)
                break;
            obstacleHitsBuffer = new Collider[obstacleHitsBuffer.Length * 2];
        }

        possibleObstacles.Clear();

        for (int i = 0; i < hitCount; i++) {
            Collider col = obstacleHitsBuffer[i];
    ...
    /// <summary>
    /// Refreshes <see cref="cornersBuffer"/> from the agent's current NavMesh path, growing the
    /// buffer if it didn't fit. Returns the corner count (0 when the agent has no path).
    /// </summary>
    ...
    private Vector3 PredictAgentFuturePosition() {
        ...
        for (int i = 0; i < count - 1; i++) {
            Vector3 closestOnSegment = GetClosestPointOnLineSegment(cornersBuffer[i], cornersBuffer[i + 1], currentPos);
        ...
        for (int i = startCorner; i < count - 1; i++) {
            Vector3 segmentStart = (i == startCorner) ?
                GetClosestPointOnLineSegment(cornersBuffer[i], cornersBuffer[i + 1], currentPos) :
                cornersBuffer[i];
            Vector3 segmentEnd = cornersBuffer[i + 1];
