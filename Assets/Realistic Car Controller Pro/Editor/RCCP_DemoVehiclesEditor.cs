...
public class RCCP_DemoVehiclesEditor : Editor {
    ...
    private void CheckProjectForPrefabs() {
        ...
        // Operate on the asset actually being inspected, not the Resources singleton —
        // they differ when a copy of the asset lives outside Resources (singleton may
        // even be null there, and a throw inside the loop would also throw in finally).
        List<RCCP_CarController> existingVehicles = prop.vehicles != null ? prop.vehicles.ToList() : new List<RCCP_CarController>();
        ...
        try {
            ...
            foreach (string path in paths) {
                ...
                if (!existingVehicles.Contains(rccp))
            ...
            if (!cancelled) {
                ...
                List<RCCP_CarController> allVehicles = existingVehicles;
