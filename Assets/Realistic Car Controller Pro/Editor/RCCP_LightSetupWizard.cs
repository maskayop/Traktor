...
public class RCCP_LightSetupWizard : EditorWindow {
    ...
    [Tooltip("Root GameObject of the vehicle being set up with lights.")]
    ...
    public class LightSetupData {
        ...
        [Tooltip("Create low-beam headlights on the vehicle.")]
        public bool addLowBeams = true;
        [Tooltip("Create high-beam headlights on the vehicle.")]
        public bool addHighBeams = false;
        [Tooltip("Create brake lights on the vehicle.")]
        public bool addBrakeLights = true;
        [Tooltip("Create always-on tail lights on the vehicle.")]
        public bool addTailLights = false;
        [Tooltip("Create reverse lights on the vehicle.")]
        public bool addReverseLights = true;
        [Tooltip("Create turn indicator lights on the vehicle.")]
        public bool addIndicators = true;

        [Tooltip("Default light intensity for headlights.")]
        ...
        [Tooltip("Default light intensity for indicator lights.")]
        public float defaultIntensityForIndicatorLights = 1f;

        [Tooltip("Color emitted by headlight sources.")]
        public Color headlightColor = new Color(1f, 1f, .9f, 1f);
        [Tooltip("Color emitted by brake light sources.")]
        public Color brakelightColor = new Color(1f, .1f, .05f, 1f);
        [Tooltip("Color emitted by tail light sources.")]
        public Color taillightColor = new Color(1f, .05f, .05f, 1f);
        [Tooltip("Color emitted by reverse light sources.")]
        public Color reverselightColor = new Color(.9f, 1f, 1f, 1f);
        [Tooltip("Color emitted by turn indicator sources.")]
        public Color indicatorColor = new Color(1f, .5f, 0f, 1f);

        [Tooltip("Attach lens flare components to generated lights.")]
        public bool useLensFlares = true;

        [Tooltip("SRP lens flare data asset for URP/HDRP pipelines.")]
        public Object lensFlareSRP;
        [Tooltip("Built-in pipeline lens flare asset.")]
        public Flare flare;
        ...
        /// </summary>
        [Tooltip("Forward offset from vehicle bounds when auto-placing lights.")]
        public float forwardOffset = .05f;
        [Tooltip("Side (lateral) offset from vehicle bounds when auto-placing lights.")]
        public float sideOffset = -.45f;
        [Tooltip("Vertical offset from vehicle bounds when auto-placing lights.")]
        public float heightOffset = -.25f;

        [Tooltip("Show interactive placement handles in the Scene view.")]
    ...
    [MenuItem("Tools/BoneCracker Games/Realistic Car Controller Pro/Vehicle Setup/Light Setup", false, 30)]
    [MenuItem("GameObject/BoneCracker Games/Realistic Car Controller Pro/Vehicle Setup/Light Setup", false, 30)]
