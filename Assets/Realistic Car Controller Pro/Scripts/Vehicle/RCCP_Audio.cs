...
public class RCCP_Audio : RCCP_Component {
    ...
    // Cached AudioClip.name strings. Object.name does a managed/native marshal on every call;
    // resolving once at Start() eliminates per-creation and per-event allocations flagged by
    // Project Auditor (PAC2002 Object.name).
    private string engineStartName;
    private string[] engineSoundOnNames;
    private string[] engineSoundOffNames;
    private string turboSoundName;
    private string blowSoundName;
    private string[] gearboxSoundNames;
    private string reverseSoundName;
    private string brakeSoundName;
    private string wheelFlatSoundName;
    private string nosSoundName;
    private string[] exhaustFlameSoundNames;
    private string windSoundName;
    private string[] crashSoundNames;
    private string wheelDeflateSoundName;
    private string wheelInflateSoundName;
    ...
    /// <summary>
    /// Resolves AudioClip.name strings once so per-creation paths don't allocate via Object.name.
    /// </summary>
    private void CacheClipNames() {

        if (engineStart != null && engineStart.audioClips != null)
            engineStartName = engineStart.audioClips.name;

        if (engineSounds != null) {

            engineSoundOnNames = new string[engineSounds.Length];
            engineSoundOffNames = new string[engineSounds.Length];
            ...
            for (int i = 0; i < engineSounds.Length; i++) {
                ...
                if (engineSounds[i].audioClipOn != null)
                    engineSoundOnNames[i] = engineSounds[i].audioClipOn.name;

                if (engineSounds[i].audioClipOff != null)
                    engineSoundOffNames[i] = engineSounds[i].audioClipOff.name;
        ...
        if (turboSound != null && turboSound.audioClips != null)
            turboSoundName = turboSound.audioClips.name;

        if (blowSound != null && blowSound.audioClips != null && blowSound.audioClips.Length > 0 && blowSound.audioClips[0] != null)
            blowSoundName = blowSound.audioClips[0].name;

        if (gearboxSound != null && gearboxSound.audioClips != null) {

            gearboxSoundNames = new string[gearboxSound.audioClips.Length];

            for (int i = 0; i < gearboxSound.audioClips.Length; i++)
                if (gearboxSound.audioClips[i] != null)
                    gearboxSoundNames[i] = gearboxSound.audioClips[i].name;

        }

        if (reverseSound != null && reverseSound.audioClips != null)
            reverseSoundName = reverseSound.audioClips.name;

        if (brakeSound != null && brakeSound.audioClips != null)
            brakeSoundName = brakeSound.audioClips.name;

        if (wheelFlatSound != null && wheelFlatSound.audioClips != null)
            wheelFlatSoundName = wheelFlatSound.audioClips.name;

        if (nosSound != null && nosSound.audioClips != null)
            nosSoundName = nosSound.audioClips.name;

        if (exhaustFlameSound != null && exhaustFlameSound.audioClips != null) {

            exhaustFlameSoundNames = new string[exhaustFlameSound.audioClips.Length];

            for (int i = 0; i < exhaustFlameSound.audioClips.Length; i++)
                if (exhaustFlameSound.audioClips[i] != null)
                    exhaustFlameSoundNames[i] = exhaustFlameSound.audioClips[i].name;

        }

        if (windSound != null && windSound.audioClips != null)
            windSoundName = windSound.audioClips.name;

        if (crashSound != null && crashSound.audioClips != null) {

            crashSoundNames = new string[crashSound.audioClips.Length];

            for (int i = 0; i < crashSound.audioClips.Length; i++)
                if (crashSound.audioClips[i] != null)
                    crashSoundNames[i] = crashSound.audioClips[i].name;

        }

        if (wheelDeflateSound != null && wheelDeflateSound.audioClips != null)
            wheelDeflateSoundName = wheelDeflateSound.audioClips.name;

        if (wheelInflateSound != null && wheelInflateSound.audioClips != null)
            wheelInflateSoundName = wheelInflateSound.audioClips.name;
            ...
            for (int i = 0; i < engineSounds.Length; i++) {
                ...
                if (!engineSounds[i].audioSourceOn && engineSounds[i].audioClipOn) {
                    ...
                    engineSounds[i].audioSourceOn = RCCP_AudioSource.NewAudioSource(
                        audioMixer,
                        gameObject,
                        engineSoundOnNames != null && i < engineSoundOnNames.Length ? engineSoundOnNames[i] : null,
                ...
                if (!engineSounds[i].audioSourceOff && engineSounds[i].audioClipOff) {
                    ...
                    engineSounds[i].audioSourceOff = RCCP_AudioSource.NewAudioSource(
                        audioMixer,
                        gameObject,
                        engineSoundOffNames != null && i < engineSoundOffNames.Length ? engineSoundOffNames[i] : null,
                        engineSounds[i].minDistance,
                        engineSounds[i].maxDistance,
                        0f,
        ...
        if (gearboxSound != null && gearboxSound.audioClips != null && gearboxSound.audioClips.Length >= 1) {
            ...
            if (lastGear != CarController.currentGear) {
                ...
                gearboxSound.audioSource = RCCP_AudioSource.NewAudioSource(
                    audioMixer,
                    gameObject,
                    gearboxSoundNames != null && randomClip < gearboxSoundNames.Length ? gearboxSoundNames[randomClip] : null,
    ...
    private void Exhaust() {
        ...
        if (cachedOtherAddons.Exhausts != null &&
            ...
            if (!exhaustFlameSound.audioSource) {
                ...
                int randomExhaustIndex = Random.Range(0, exhaustFlameSound.audioClips.Length);
                AudioClip randomExhaustClip = exhaustFlameSound.audioClips[randomExhaustIndex];

                exhaustFlameSound.audioSource = RCCP_AudioSource.NewAudioSource(
                    audioMixer,
                    gameObject,
                    exhaustFlameSoundNames != null && randomExhaustIndex < exhaustFlameSoundNames.Length ? exhaustFlameSoundNames[randomExhaustIndex] : null,
                ...
                // Check if any exhaust plays its own spatialized backfire sound
                bool exhaustPlaysSound = false;
                foreach (var ex in cachedOtherAddons.Exhausts.Exhaust) {
                    if (ex != null && ex.playBackfireSound) {
                        exhaustPlaysSound = true;
                        break;
                    }
                }

                // Play backfire sound when exhaust is popping and no exhaust is playing its own spatialized sound
                if (!exhaustPlaysSound && !exhaustFlameSound.audioSource.isPlaying && isPopping) {
                ...
                if (exhaustFlameSound.audioSource.isPlaying && (!isPopping || exhaustPlaysSound)) {
        ...
        if (crashSound != null && crashSound.audioClips != null && crashSound.audioClips.Length >= 1) {
            ...
            if (volume > AUDIO_DEADZONE) {
                ...
                AudioSource newSource = RCCP_AudioSource.NewAudioSource(
                    audioMixer,
                    gameObject,
                    crashSoundNames != null && randomClip < crashSoundNames.Length ? crashSoundNames[randomClip] : null,
