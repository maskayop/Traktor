...
public class RCCP_UI_RebindInput : RCCP_UIComponent {
    ...
    public bool ResolveActionAndBinding(out InputAction action, out int bindingIndex) {
        ...
        if (bindingIndex == -1) {
            // Pre-stringify the Guid so the interpolation params[] doesn't box the struct.
            Debug.LogError("Cannot find binding with ID '" + bindingId.ToString() + "' on '" + action + "'", this);
            return false;
        }
    ...
    public void UpdateBindingDisplay() {
        ...
        if (action != null && Guid.TryParse(m_BindingId, out var bindingId)) {
            // Compare Guid==Guid instead of stringifying each element's id — IndexOf walks every
            // binding and the original form allocated a temporary string per iteration.
            var bindingIndex = action.bindings.IndexOf(x => x.id == bindingId);
            if (bindingIndex != -1)
