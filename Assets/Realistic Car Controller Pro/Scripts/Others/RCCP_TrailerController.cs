...
public class RCCP_TrailerController : RCCP_GenericComponent {
    ...
    /// </summary>
    [Tooltip("Vehicle controller currently towing this trailer (set automatically on attach).")]
    ...
    public class TrailerWheel {
        ...
        /// </summary>
        [Tooltip("WheelCollider component driving this trailer wheel's physics.")]
        public WheelCollider wheelCollider;

        /// <summary>
        /// The visual model (mesh) for this wheel.
        /// </summary>
        [Tooltip("Visual mesh transform that follows the WheelCollider pose.")]
        public Transform wheelModel;

        /// <summary>
        /// Determines if this wheel should steer when the trailer moves or turns.
        /// </summary>
        [Tooltip("Whether this wheel steers based on sideways slip when the trailer turns.")]
        public bool steering = false;

        /// <summary>
        /// The maximum steer angle for this wheel if steering is enabled.
        /// </summary>
        [Tooltip("Maximum steering angle in degrees when slip-based steering is enabled.")]
    ...
    /// </summary>
    [Tooltip("All wheels on this trailer, each with its own collider and visual model.")]
    ...
    /// </summary>
    [Tooltip("Transform defining the trailer's center of mass position.")]
    ...
    /// </summary>
    [Tooltip("Support legs GameObject shown when the trailer is detached.")]
    ...
    /// </summary>
    [Tooltip("Trigger-based attacher component used to connect this trailer to vehicles.")]
    public RCCP_TrailerAttacher attacher;

    /// <summary>
    /// Indicates if the trailer is attached to a towing vehicle (joint.connectedBody != null).
    /// </summary>
    [Tooltip("Whether this trailer is currently attached to a towing vehicle.")]
    ...
    /// </summary>
    [Space()]
    [Tooltip("Override the TPS camera distance and height when this trailer is attached.")]
    ...
    /// </summary>
    [Tooltip("TPS camera distance from the vehicle when manually overridden.")]
    [Min(0f)] public float TPSDistance = 20f;

    /// <summary>
    /// Third-Person-Style camera height when manually setting camera distance and height.
    /// </summary>
    [Tooltip("TPS camera height above the vehicle when manually overridden.")]
    [Min(0f)] public float TPSHeight = 6f;
    ...
    private class JointRestrictions {
        ...
        /// </summary>
        [Tooltip("Angular Y-axis rotation constraint of the trailer joint.")]
        ...
        /// </summary>
        [Tooltip("Angular Z-axis rotation constraint of the trailer joint.")]
    ...
    /// </summary>
    [Space()]
    [Tooltip("Maximum rotation angle in degrees between trailer and towing vehicle before correction forces apply.")]
    [Min(0f)] public float maxAngle = 90f; // Maximum allowed rotation in degrees
    ...
    /// <summary>The transform used as the hitch/connection point for attaching to a towing vehicle.</summary>
    ...
    /// <summary>Serialized reference to the trailer's hitch connection point transform.</summary>
    [Tooltip("Transform marking the hitch point where the trailer connects to the towing vehicle.")]
    public Transform trailerConnectionPoint;
    ...
    private void Start() {
        ...
        if (Joint.connectedBody) {
            Joint.connectedBody.gameObject.TryGetComponent<RCCP_CarController>(out var connectedCar);
            AttachTrailer(connectedCar);
        } else {
            DetachTrailer();
        }
    ...
    private void Reset() {
        ...
        if (!TryGetComponent<Rigidbody>(out var rigidb))
